...
def kepler1_key_v1(event):
    global kp1_strecke_sonne, kp1_x, kp1_y, r_abstand, kp1_key_press, kp1_durchlauf, kp1_durchschnitt, zwischenstrecken_index, kp1_strecke_bp, kp1_durchschnitt_v2
    if event.key == 'y':
        if runde_gemacht and not kp1_durchlauf:
            r_abstände.clear()
...
def kepler1_v1():
    global kp1_strecke_sonne, kp1_key_press, brennpunkte, r_abstand_halb, r_abstand_mes, kp1_mes
    if kp1_key_press:
        if kp1_strecke_sonne < (umlaufsstrecke /2):
            ...
            zwischenstrecken.append(kp1_strecke_sonne)
...
def kepler1durchschnitt_v1 ():
    global kp1_strecke_bp, zwischenstrecken_index, kp1_durchschnitt, kp1_mes, kp1_durchlauf, std_v1, r_abstände_total
    if kp1_mes:
        x1 = r_interp_fiktiv[0, current_frame] / AE
        y1 = r_interp_fiktiv[1, current_frame] / AE
        x2 = r_interp_fiktiv[0, current_frame + 1] / AE
        ...
        if len(r_abstände) == len(r_abstände_halb):
            ...
            #Standartabweichung
            std_v1 = np.std(r_abstände_total)
            std_v1_th = np.sqrt(np.mean((np.array(r_abstände_total) - (r_aphel + r_perihel) / AE)**2))
...
def durchschnitt_kepler1_v2():
    global kp1_durchschnitt_v2, std_v2
    if len(r_x_punkte) != 0 and len(r_x_punkte) != len(r_anstände_total_v2):
        x = r_interp_fiktiv[0, current_frame] / AE
        y = r_interp_fiktiv[1, current_frame] / AE
    ...
    elif kp1_durchschnitt_v2 == 0 and len(r_anstände_total_v2) != 0:
        kp1_durchschnitt_v2 = np.mean(r_anstände_total_v2)

        #Standartabweichung
        std_v2 = np.std(r_anstände_total_v2)
        std_v2_th = np.sqrt(np.mean((np.array(r_anstände_total_v2) - (r_aphel + r_perihel) / AE)**2))
...
def update_kepler_1(n):
    ...
    a_kepler_1_Rechung.set_text(f"Rechnung: {(r_aphel + r_perihel) / AE:.2f} AE")
    a_kepler_1_Messung_v1.set_text(f"Messung: {(r_abstand_mes):.4f} AE")
    a_kepler_1_Messung_v2.set_text(f"Messung v2: {(r_abstand_mes_v2):.4f} AE")
    a_kepler_1_Durchschnitt_v1.set_text(f"Durchschnitt: {(kp1_durchschnitt):.4f} AE")
    a_kepler_1_Durchschnitt_v2.set_text(f"Durchschnitt v2: {(kp1_durchschnitt_v2):.4f} AE")
...
def update_kepler_2(n):
    ...
    a_kepler_2.set_text(f"Abschnittsfläche: {fläche_rot * AE * AE:.2e} $m^2$")
    a_kepler_2_strecke_rot.set_text(f"Strecke der roten Abschnittsfläche: {strecke_rot * AE:.2e} $m$")
    a_kepler_2_strecke_magenta.set_text(f"Strecke der magenta Abschnittsfläche: {strecke_magenta * AE:.2e} $m$")
    a_kepler_2_zeit.set_text(f"Benötigte Zeit: {t_rot} $Tage$")
    a_kepler_2_fiktiv.set_text(f"Abschnittsfläche fiktiv: {fläche_grau * AE * AE:.2e} $m^2$")
    a_kepler_2_strecke_grau.set_text(f"Strecke der grauen Abschnittsfläche: {strecke_grau * AE:.2e} $m$")
    a_kepler_2_strecke_schwarz.set_text(f"Strecke der schwarzen Abschnittsfläche: {strecke_schwarz * AE:.2e} $m$")
